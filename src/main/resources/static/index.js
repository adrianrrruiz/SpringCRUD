function cargaEstudiantes() {
    ...
    xhttp.open("GET", "http://localhost:8081/api/estudiantes");
    xhttp.send();
    xhttp.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        ...
        for (let object of objects) {
          ...
          trHTML +=
            '<button type="button" class="btn btn-outline-info" onclick="verNotas(' + object["id"] + ',\'' + object["nombre"]+ '\', \'' + object["apellido"] +' \')">Notas</button>';
          trHTML +=
  ...
  function creaEstudiante() {
    ...
    const xhttp = new XMLHttpRequest();
    xhttp.open("POST", "http://localhost:8081/api/crea");
    ...
    xhttp.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        const objects = JSON.parse(this.responseText);
        Swal.fire("El estudiante se creó correctamente :)");
        cargaEstudiantes();
      }
    };
  }
  ...
  function verNotas(id, nombre, apellido){
    window.location.href = "notas.html?id=" + id + "&nombre=" + nombre + "&apellido=" + apellido;
  }
  ...
  function editarEstudiante(id) {
    ...
    const xhttp = new XMLHttpRequest();
    xhttp.open("PUT", "http://localhost:8081/api/act/" + id );
    xhttp.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
    xhttp.send(
      JSON.stringify({
        id: id,
    ...
    xhttp.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        const objects = JSON.parse(this.responseText);
        Swal.fire("El estudiante se actualizó correctamente :)");
        cargaEstudiantes();
      }
    };
  }
  function borrarEstudiante(id) {
    const xhttp = new XMLHttpRequest();
    xhttp.open("DELETE", "http://localhost:8081/api/borra/" + id );
    xhttp.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
    xhttp.send(
      JSON.stringify({
        id: id,
